 ...
 * before React ever mounts. A failed boot would otherwise render a silent blank
 * panel. This handler turns that exact failure mode into a visible message.
 */
window.addEventListener('error', (event) => {
  const root = document.getElementById('root');
  if (root !== null && root.childNodes.length === 0) {
    const message = event.message ?? 'The side panel failed to start.';
